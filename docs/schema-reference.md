...
| `version` | integer | yes | Settings format version. Current version is `4`. |
...
`omni tools fallback <tool> --from-github owner/repo` only when Omni can resolve
latest stable GitHub release metadata and a supported asset for the current
platform. If resolution fails, the command leaves existing config unchanged.
