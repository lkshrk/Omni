...
# grep may find no tag; let the version guard print the diagnostic under pipefail.
...
# Use a temporary directory so installation never writes into the caller's working directory.
