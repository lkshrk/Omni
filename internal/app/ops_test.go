...
func TestRemoveToolFromConfig_RemovesConcreteConfiguredCacheForLogicalProvider(t *testing.T) {
	...
	if err := a.DB().Upsert(context.Background(), &database.ToolCache{
		...
		t.Fatalf("seed concrete cache: %v", err)
	}

	if err := a.RemoveToolFromConfig(context.Background(), "rg", "system"); err != nil {
		t.Fatalf("RemoveToolFromConfig: %v", err)
	}

	if _, err := a.DB().Get(context.Background(), "rg", "apt", "ripgrep"); err == nil || !errors.Is(err, sql.ErrNoRows) {
		t.Fatalf("concrete cache row err = %v, want sql.ErrNoRows", err)
	}
	cfg, err := config.Load(cfgPath)
	if err != nil {
		t.Fatalf("config.Load: %v", err)
	}
	if hasTool(cfg, "rg", "system") {
...
func TestInstallProviderMatches_AllowWeakProviderFamilyFilterInstallsConcreteMatch(t *testing.T) {
	...
	result, err := a.InstallProviderMatches(context.Background(), "prettier", "system", app.ProviderMatchOptions{AllowWeak: true})
	if err != nil {
		t.Fatalf("InstallProviderMatches: %v", err)
	}
	if result.Installed.Provider != "brew" || result.Installed.Package != "prettier-plugin-tailwindcss" {
...
func TestInstallProviderMatches_AllowWeakNodeFamilyFilterInstallsConcreteMatch(t *testing.T) {
	...
	result, err := a.InstallProviderMatches(context.Background(), "prettier", "node", app.ProviderMatchOptions{AllowWeak: true})
	if err != nil {
		t.Fatalf("InstallProviderMatches: %v", err)
	}
	if result.Installed.Provider != "npm" || result.Installed.Package != "prettier-plugin-tailwindcss" {
...
func TestSync_AllowWeakHonorsProviderFilter(t *testing.T) {
	npm := &searchStub{
		stubProvider: stubProvider{name: "npm", available: true},
		results: []provider.SearchResult{{
			Name:     "prettier-plugin-tailwindcss",
	...
	result, err := a.Sync(context.Background(), syncprogress.SyncOptions{Provider: "brew", AllowWeak: true})
	if err != nil {
		t.Fatalf("Sync: %v", err)
	}
	if installed := result.Installed(); len(installed) != 1 || installed[0].Tool.Provider != "brew" || installed[0].Tool.Package != "prettier-plugin-tailwindcss" {
		t.Fatalf("installed = %+v, want filtered weak brew package install", installed)
	}
	cfg, err := config.Load(cfgPath)
	if err != nil {
		t.Fatalf("config.Load: %v", err)
