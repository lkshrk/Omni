...
func newDNF(responses ...executor.MockCall) (*dnf.Provider, *executor.MockExecutor) {
...
func TestAvailable_False(t *testing.T) {
	p, _ := newDNF(executor.MockCall{Err: errors.New("not found")})
	ok, err := p.Available(context.Background())
	if err != nil || ok {
...
func TestInstall_Error(t *testing.T) {
	p, _ := newDNF(executor.MockCall{Err: errors.New("exit 1"), Stderr: "No match for argument"})
	if err := p.Install(context.Background(), tool("badpkg")); err == nil {
		t.Fatal("expected error, got nil")
	}
}

func TestInstall_ErrorPropagates(t *testing.T) {
...
func TestName(t *testing.T) {
	p, _ := newDNF()
	if p.Name() != "dnf" {
		t.Errorf("Name() = %q, want %q", p.Name(), "dnf")
	}
}
...
func TestIsInstalled_Found(t *testing.T) {
	...
	if err != nil || !ok || ver != "14.1.1-4.fc44" {
		t.Errorf("IsInstalled() = (%v, %q, %v), want (true, 14.1.1-4.fc44, nil)", ok, ver, err)
	}
}
...
func TestListInstalled(t *testing.T) {
	...
	p, m := newDNF(executor.MockCall{Stdout: output})
	...
	if tools[0].Name != "ripgrep" || tools[0].Version != "14.1.1-4.fc44" {
		t.Errorf("unexpected tool[0]: %+v", tools[0])
	}
	if len(m.Calls) != 1 {
		t.Fatalf("repoquery calls = %#v, want one call", m.Calls)
	}
	args := m.Calls[0].Args
	if len(args) == 0 || args[len(args)-1] != "%{name}\t%{evr}\n" {
		t.Fatalf("repoquery format = %#v, want newline-terminated queryformat", m.Calls)
	}
}
...
func TestInstalledMap(t *testing.T) {
	...
	if v, ok := m["ripgrep"]; !ok || v != "14.1.1-4.fc44" {
...
func TestParseRPMLine(t *testing.T) {
	tests := []struct {
		...
		{"ripgrep\t14.1.1-4.fc44", "ripgrep", "14.1.1-4.fc44"},
		{"", "", ""},
		{"noversion", "", ""},
	}
	for _, tc := range tests {
		p, _ := newDNF(executor.MockCall{Stdout: tc.line + "\n"})
...
func TestListInstalledSurfacesCommandOutputDetail(t *testing.T) {
	...
	p, _ := newDNF(executor.MockCall{Err: sentinel, Stderr: "boom: repo unreachable\n"})
...
func TestListInstalledSurfacesStdoutDetailWhenStderrEmpty(t *testing.T) {
	sentinel := errors.New("exit status 1")
	p, _ := newDNF(executor.MockCall{Err: sentinel, Stdout: "fail written to stdout\n"})
	if _, err := p.ListInstalled(context.Background()); err == nil || !errors.Is(err, sentinel) {
		t.Fatalf("ListInstalled() error = %v, want wrapped sentinel", err)
