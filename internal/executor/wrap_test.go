...
func TestWrapErrorIncludesBothStreams(t *testing.T) {
...
func TestWrapErrorTruncatesStreamsIndependently(t *testing.T) {
	stdout := strings.Repeat("progress line\n", 400) + "final stdout error"
	err := WrapError(errors.New("exit status 1"), "apm install", stdout, "fatal stderr")
	got := err.Error()
	if !strings.Contains(got, "fatal stderr") || !strings.HasSuffix(got, "final stdout error") {
		t.Fatalf("error lost a stream: %q", got)
	}
}
