...
func TestAgentsViewShowsLastAPMResult(t *testing.T) {
	...
	m.apmOutput = "┌──────┐\n│ pkg  │"
	m.apmNotices = []string{"warning: template not applied"}
	m.apmErr = errors.New("audit failed")
	view := m.viewSkillsBody()
	for _, want := range []string{"~/.apm/apm.yml", "omni agents sync", "warning: template not applied", "audit failed"} {
		if !strings.Contains(view, want) {
			t.Fatalf("view missing %q:\n%s", want, view)
		}
	}
	if strings.Contains(view, "│ pkg") {
		t.Fatalf("raw APM table rendered:\n%s", view)
	}
}
