...
func resolveOnboardItem(item *apm.ImportItem, key string) {
	if item == nil {
		return
	}
	switch key {
	...
	case "l":
		item.Resolution.Decision = "import"
		item.Resolution.ApprovedTargets = []string{"claude"}
	case "b":
		item.Resolution.Decision = "import"
		item.Resolution.ApprovedTargets = []string{"claude", "codex"}
	case "x":
