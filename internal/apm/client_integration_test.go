...
func requirePinnedAPM(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("integration tests require apm 0.28.0+omni.8 on PATH: %v", err)
	}
	...
	if !slices.Contains(fields, "0.28.0+omni.8") {
		t.Fatalf("integration tests require exactly apm 0.28.0+omni.8, got %q", strings.TrimSpace(string(output)))
	}
}
